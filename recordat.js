...
// AQUI EMPIEZA JS DE RECORDATORIOS Tasks Container
...
const addNewTask = event => {
    ...
    task
        .classList
        .add('task', 'roundBorder');
    task.addEventListener('click', changeTaskState);
    task.innerHTML = `
        <span>${value}</span>
        <button class="deleteTaskButton" onclick="deleteTask(event)">Eliminar</button>
    `;
...
const loadTasksFromLocalStorage = () => {
    ...
    if (savedTasks) {
        ...
        // Configurar eventos para las tareas recién cargadas
        tasksContainer
...
// function to add days in days with class day and prev-date next-date on
// previous month and next month days and active on today
...
addEventTitle.addEventListener("input", (e) => {
    addEventTitle.value = addEventTitle
        .value
        .slice(0, 60);
});
...
addEventFrom.addEventListener("input", (e) => {
    addEventFrom.value = addEventFrom
        .value
        .replace(/[^0-9:]/g, "");
...
function getEvents() {
    // check if events are already saved in local storage then return event else
    // nothing
